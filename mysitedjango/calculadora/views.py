...
def home(request):
    ...
    if request.method == "POST":
        ...
        punto_a = float(punto_a_input) if punto_a_input else 0

        # Llamamos a la lógica principal
        resultado = calcular_taylor_maclaurin(funcion, punto_a)

        # Si no hay errores, generamos la gráfica
        if resultado and not resultado.get('error'):
            serie_obj = resultado['resultado_final_raw']
            resultado['grafica'] = generar_grafica(funcion, punto_a, serie_obj)
